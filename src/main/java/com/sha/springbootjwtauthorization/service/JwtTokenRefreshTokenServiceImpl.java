...
@Service
@Component
public class JwtTokenRefreshTokenServiceImpl implements JwtTokenRefreshService{

    @Value("${app.jwt.refresh_expiration_in_ms}")
   ...
   @Override
    public JwtRefreshToken createRefreshToken(Long userId)
    {
        ...
        jwtRefreshToken.setExpirationDt(LocalDateTime.now().plus(REFRESH_EXPIRATION_IN_MS, ChronoUnit.MILLIS));
        jwtRefreshToken.setCreateDt(LocalDateTime.now());
    ...
    public User generateAccessTokenFromRefreshToken(String refreshTokenId)
    {
        ...
        if(jwtRefreshToken.getExpirationDt().isBefore(LocalDateTime.now()))
        {
